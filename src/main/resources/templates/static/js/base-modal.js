...
(function ($, window, document, undefined) {
    ...
    function _modal_structure(config, ok, cancel) {
        ...
        if (ok || cancel || config.has_footer) {
            ...
            if (cancel) {
                ...
                btn_close.innerHTML = config.title == '提示' ? '确定' : config.cancel_label;
    ...
    var _TITLE = { info: '提示', correct: '提示', warn: '警告', error: '错误' };
    ...
    var _win_config={winId:'user-win',top:'auto',backdrop:true,keyboard:true,width:900};
    ...
    modals.showLoading=function(e){
	   	...
	   	jQuery(e).showLoading({indicatorID:'showload_myindicatorId'});
    }
    
    modals.hideLoading=function(e){
	   	if(!e){
	   		var _content = $('.content');
	   		if(_content && _content.length == 1){
	   			e = _content;
	   	...
	   	jQuery(e).hideLoading({indicatorID:'showload_myindicatorId'});
    }
